/**
 * Backfill 30 days of history to Vercel Blob.
 * Requires BLOB_READ_WRITE_TOKEN and OPENELECTRICITY_API_KEY in env.
 */
...
function buildIntervalBitmap(
	...
	const startMs = new Date(`${dayStartAest}Z`).getTime()
...
async function main() {
	...
	const history: StatusHistory = { v: 1, updatedAt: new Date().toISOString(), series: {} }
	...
	for (let dayOffset = BACKFILL_DAYS - 1; dayOffset >= 0; dayOffset--) {
		...
		const dateEnd = dayOffset === 0 ? nowAest.toISOString().slice(0, 19) : `${dateStr}T23:59:59`
	...
	// Also build current.json from latest data
	const { computeCurrentStatus } = await import("../src/lib/compute")
	const now = new Date()
	...
	const [networkResponse, marketResponse] = await Promise.all([
		client.getNetworkData("NEM", ["power", "emissions"], {
			...
			primaryGrouping: "network_region",
			secondaryGrouping: ["fueltech_group"],
			dateStart: toAestNaive(windowStart),
			dateEnd: toAestNaive(now),
		}),
		client.getMarket("NEM", ["price", "demand"], {
			interval: "5m",
			primaryGrouping: "network_region",
	...
	console.log("Writing to Vercel Blob...")
	await Promise.all([writeBlob("current.json", current), writeBlob("history.json", history)])
	console.log("Done — wrote current.json + history.json to blob")

	for (const def of SERIES_DEFINITIONS) {
		const days = history.series[def.id] ?? []
		console.log(`  ${def.id}: ${days.length} days`)
	}
}
